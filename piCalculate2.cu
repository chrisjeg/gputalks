...
randArray[idx] = curand_uniform(&state[idx]) - 0.5f;
}
...
double gpu_calc_pi_monte_carlo(int samples){
    ...
    // setup seeds
    initialise_curand_on_kernels<<<blocks, threads>>>(d_state, unsigned(time(NULL)));
    set_random_number_from_kernels<<<blocks, threads>>>(d_x, d_state, samples);
    set_random_number_from_kernels<<<blocks, threads>>>(d_y, d_state, samples);
    cudaFree(d_state);
    //init_stuff<<<blocks, threads>>>(d_state,time(NULL));
    //make_rand<<<blocks, threads>>>(d_state, d_x);
    //init_stuff<<<blocks, threads>>>(d_state,time(NULL));
    //make_rand<<<blocks, threads>>>(d_state, d_y);
